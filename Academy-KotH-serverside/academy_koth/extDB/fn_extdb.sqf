...
if ( isNil {uiNamespace getVariable "extDB_koth_ID"}) then
{
	...
	extDB_koth_ID = compileFinal _extDB_SQL_CUSTOM_ID;
	...
	if (_result select 0 isEqualTo 0) exitWith {diag_log format ["extDB2: Error START_RCON:RCON: %1", _result]; false};
	_result = call compile ("extDB2" callExtension "9:ADD_PROTOCOL:RCON:rcon");
	if (_result select 0 isEqualTo 0) exitWith {diag_log format ["extDB2: Error ADD_PROTOCOL:RCON: %1", _result]; false};
	uiSleep 1;

	_result = call compile ("extDB2" callExtension "9:RCON_STATUS");
	diag_log format ["extDB2: STATUS_RCON: %1 (1=Online, 2=Offline)", _result];
	...
	// Save Randomized ID
	uiNamespace setVariable ["extDB_koth_ID", _extDB_SQL_CUSTOM_ID];
	_return = true;
}
else
{
